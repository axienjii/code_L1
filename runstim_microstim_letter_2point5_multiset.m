function runstim_microstim_letter_2point5_multiset(Hnd)
%Written by Xing 15/5/18
%Present 2 targets for letter task. Letters are: A and L (top and bottom targets, respectively). 
%The value of setInd is selected on each trial, e.g. between 44 and 62.
%Hence, the set of electrodes that is used to depict a given letter varies from trial to trial.
...
setIndsRange=64:68;
trialConds=[ones(1,length(setIndsRange)) ones(1,length(setIndsRange))*2];%trial conditions. Target conds in first row: for TB trials, 1: target is above; 2: target is below
trialConds=[trialConds;[repmat(setIndsRange,[1,1]) repmat(setIndsRange,[1,1])]];
...
load('C:\Users\Xing\Lick\currentThresholdChs126.mat');%increased threshold for electrode 51, array 10 from 48 to 108, adjusted thresholds on all 4 electrodes
