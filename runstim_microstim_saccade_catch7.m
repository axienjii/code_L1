function runstim_microstim_saccade_catch7(Hnd)
...
%Implements Quest, using log10 values.
...
global allQ
global questCounterHit
global questCounterMiss
global allStimCurrentLevel
global currentAmplitudeUpperBound
global currentAmplitudeLowerBound
global firstLowerBoundFlag
...
RACT = 1000;      %reaction time 250 ms
...
questCounterHit=0;
questCounterMiss=0;
currentAmplitudeLowerBound=0;

electrodeInd=26;%electrodes 37 and 38 (indices 12 & 13, respectively) on array 13
...
currentAmplitude=110;
currentAmplitudeUpperBound=210;
firstLowerBoundFlag=1;
...
%         disp(FIXT);
    ...
    if Hit ~= 0 && ~Abort %has entered a target window (false or correct)
        ...
        if hitCounter==2%if two hits accrued
            currentAmplitudeUpperBound=currentAmplitude;
            if firstLowerBoundFlag==1
                currentAmplitude=currentAmplitude/2;%decrease current
                firstLowerBoundFlag=0;
            elseif firstLowerBoundFlag==0
                currentAmplitude=(currentAmplitude+currentAmplitudeLowerBound)/2;%decrease current
            end
            currentAmplitudeLowerBound=currentAmplitude;
        ...
        elseif missCounter==2%if two misses accrued
            if firstLowerBoundFlag==1
                currentAmplitudeLowerBound=currentAmplitude;
                currentAmplitude=(currentAmplitudeUpperBound+currentAmplitude)/2;%increase current
                firstLowerBoundFlag=0;
            elseif firstLowerBoundFlag==0
                currentAmplitude=(currentAmplitudeUpperBound+currentAmplitudeLowerBound)/2;%increase current
            end
            currentAmplitudeLowerBound=currentAmplitude;
        ...
        if currentAmplitude>0%miss trial
            ...
            elseif questCounterMiss==1%if one misses occurred
                response=0;%feed 'miss' response into Quest
            end
            if questCounterHit==2||questCounterMiss==1
