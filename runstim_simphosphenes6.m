function runstim_simphosphenes6(Hnd)
   ...
   if Hit == 0 %subject kept fixation, display stimulus
       ...
       if Hit == 0 %subject kept fixation, display stimulus
           ...
           if Hit == 0 %subject kept fixation, subject may make an eye movement
               disp('target on')
               %Set parameters for drawing target
               ...
               elseif letterCond==length(allLetters(:))+1
                   for i=1:size(allLetters,1)-1
               ...
               elseif letterCond==length(allLetters(:))+2
                   for i=1:size(allLetters,1)-1
                       Screen('FillRect',w,targcol,[screenWidth/2-estimatedTargetLetterSize/2+distx(i),screenHeight/2-estimatedTargetLetterSize/2+disty(i),screenWidth/2+estimatedTargetLetterSize/2+distx(i),screenHeight/2+estimatedTargetLetterSize/2+disty(i)]);
                   end
                   Screen('FillRect',w,targcol,[screenWidth/2-estimatedTargetLetterSize/2+targetArrayX(targetLocation),screenHeight/2-estimatedTargetLetterSize/2+targetArrayY(targetLocation),screenWidth/2+estimatedTargetLetterSize/2+targetArrayX(targetLocation),screenHeight/2+estimatedTargetLetterSize/2+targetArrayY(targetLocation)]);
               end
