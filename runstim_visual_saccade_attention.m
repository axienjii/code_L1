function runstim_visual_saccade_attention(Hnd)
%Written by Xing 24/7/18
%Attention task in which the monkey attends to a visually presented
%percept, either on the left or the right side of the screen. The two attention
%conditions are blocked. The monkey has to fixate for 300 ms, at which point a 
...
%Modified from runstim_microstim_saccaede_attention9.m, in which the
%percept on the right side was not a visually presented dot, but a
%phosphene. In previous version:
...
%On this version, a visual stimulus is presented on both sides, to check
%whether attention modulation is visible.
...
%         disp(FIXT);
        while Time < FIXT && Hit== 0
            ...
            if distractFirst==1&&initialTrial==0
                ...
                if visualTrial==1%distractor visual stimulus in the right hemifield
    ...
    if Hit == 0 %subject kept fixation, display stimulus
        ...
        if visualTrial==1&&stimFlag2==1%visual target on the left
            ...
            Screen('Flip', w);
            stimFlag2=0;
        elseif visualTrial==0&&stimFlag==1%visual target on the right
            %draw line composed of series of simulated phosphenes
            for phospheneInd=1:numSimPhosphenes
                destRect=[screenWidth/2+RFx(phospheneInd)-ceil(diameterSimPhosphenes(phospheneInd)/2) screenHeight/2-RFy(phospheneInd)-ceil(diameterSimPhosphenes(phospheneInd)/2) screenWidth/2+RFx(phospheneInd)+ceil(diameterSimPhosphenes(phospheneInd)/2) screenHeight/2-RFy(phospheneInd)+ceil(diameterSimPhosphenes(phospheneInd)/2)];
                Screen('DrawTexture',w, masktex(phospheneInd), [], destRect);
