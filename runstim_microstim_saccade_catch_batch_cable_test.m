function runstim_microstim_saccade_catch_batch_cable_test(Hnd)
%Written by Xing 18/1/18
%Used to test quality of signal from monitor port (module 1), using various
%cables (BNC or flat cable).
...
allHitY=[];
allHitRT=[];
currentValsHardCoded=[1 2 3 4 5 6 7 8];
subblockCount=0;
...
trialsDesired=5;
trialConds=repmat(1:5,1,trialsDesired);%trial conditions. Target conds in first row: for TB trials, 1: target is above; 2: target is below
...
%         currentAmplitude=finalCurrentVals(currentInd);
        currentAmplitude=currentValsHardCoded(condOrder(1));
        % define a waveform
        waveform_id = 1;
...
%     end
    ...
    if numHitsElectrode+numMissesElectrode>=trialsDesired*length(currentValsHardCoded)%sum(logical(diff(allStaircaseResponse)))>=minNumReversals%||(numHitsElectrode/numMissesElectrode<0.1&&numHitsElectrode+numMissesElectrode>=50)||numHitsElectrode+numMissesElectrode>80%if there are min num of reversals, or the proportion of hits to misses is low after a sufficient number of trials, terminate staircase procedure
        allStaircaseResponse=[];
