function runstim_microstim_RF_vs_saccade(Hnd)
%Written by Xing 29/1/18
%Calibrate saccadic endpoints between visual and phosphene targets, and
%compare saccade endpoints with locations of RF centres. 
...
visDotsz = visDotSize.*Par.PixPerDeg;
...
visual=1;%set to 1 to present simulated phosphene visually onscreen; set to 0 to send microstimulation pulse
if visual==0
    trialsDesired=15;
elseif visual==1
    trialsDesired=10;
...
while ~Par.ESC%&&electrodeNumInd<=length(electrodeNums)
    ...
    elseif catchTrial==0
        ...
        currentAmplitude=ceil(existingThreshold*1.5);
        if visual==0
            % define a waveform
            waveform_id = 1;
...
%                 Screen('FillOval',w,[0 255 0],[Par.HW-Fsz/2 Par.HH-Fsz/2 Par.HW+Fsz Par.HH+Fsz]);
                Screen('FillOval',w,[255 255 255],[screenWidth/2+RFx-visDotsz/2 screenHeight/2-RFy-visDotsz/2 screenWidth/2+RFx+visDotsz screenHeight/2-RFy+visDotsz]);
                Screen('Flip', w);                
            end
        end
...
%     end
    ...
    if numHitsElectrode>=trialsDesired||numMissesElectrode-numHitsElectrode>20
        allStaircaseResponse=[];
