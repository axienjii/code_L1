function runstim_microstim_saccade_attention5(Hnd)
...
while ~Par.ESC&&staircaseFinishedFlag==0%&&electrodeNumInd<=length(electrodeNums)
    ...
    if newBlock==1
        ...
        trialConds=[zeros(1,trialsDesired+20) ones(1,trialsDesired+20)];%trial conditions. 1: visual target, on left; 2: phosphene target, on right
        distractTrialsV=randperm(trialsDesired);
        distractTrialsV(find(distractTrialsV<=trialsDesired/5))=1;
        distractTrialsV(find(distractTrialsV>trialsDesired/5))=0;
        distractTrialsM=randperm(trialsDesired);
        distractTrialsM(find(distractTrialsM<=trialsDesired/5))=1;
        distractTrialsM(find(distractTrialsM>trialsDesired/5))=0;
        trialConds=[trialConds;zeros(1,20) distractTrialsV zeros(1,20) distractTrialsM];%second row in trialConds controls whether distractor is present (0) or not (1)
