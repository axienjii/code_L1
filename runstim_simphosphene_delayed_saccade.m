function runstim_simphosphene_delayed_saccade(Hnd)
%Written by Xing 10/10/16
%Present two simulated phosphenes, either aligned or offset in the 
...
%REactie tijd
TARGT = 350; %time to keep fixating after target onset before fix goes green (het liefst 400)
...
while ~Par.ESC
    ...
    if Hit ~= 0  %subjects eyes are in fixation window keep fixating for FIX time
        ...
        Time = 1;
        Hit = 0;
        FIXT=random('unif',750,950);%1000,2300
        disp(FIXT);
        stim_on_flag=0;
        while Time < FIXT && Hit== 0
            %Check for 10 ms
            dasrun(5)
            stimDuration=randi([200 450]);
