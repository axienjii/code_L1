...
%     setInd=condOrderSet(1);
    ...
    if stimPattern>2
        if mod(stimPattern,2)==1%instead of sequence with stimulation on outermost two first, stimulate on innermost two first
...
%                 phospheneCol=[0 0 0];
    ...
    elseif visualTrial==0        
        ...
        switch(stimPattern)%set number of mini trains
            case 1
                numRepeats=1;
            case 2
                numRepeats=1;
        ...
        pulsesPerMiniTrain=50/numRepeats;
        if stimPattern==1||stimPattern==2
            send_stim_multiple_CereStims_no_wait(uniqueStimulators,currentAmplitudeSequenceInd,electrodeSequenceInd,stimulatorNums,stimulator,stimSequenceInd)%send stimulation simultaneously on both electrodes
        else
            send_stim_multiple_CereStims_interleaved_12(uniqueStimulators,currentAmplitudeSequenceInd,electrodeSequenceInd,stimulatorNums,stimulator,stimSequenceInd,stimPattern,pulsesPerMiniTrain)
        end
    end
...
%                             Screen('Flip', w);
                ...
                elseif stimPattern>=3
                    dasbit(Par.MicroB,1);%send the trigger signal
%                     for uniqueStimInd=2%:length(uniqueStimulators)%use this code to interleave pulses between electrodes on a given CereStim (and to a lesser extent, between electrodes on different CereStims)
%                         stimulatorInd=find(stimulatorNums==uniqueStimulators(uniqueStimInd));            
%                         XXXXX = stimulator(stimulatorInd).getSequenceStatus;
%                         disp('Sequence Status:');
%                         disp(XXXXX);
%                         stimulator(stimulatorInd).play(numRepeats);
%                         XXXXX = stimulator(stimulatorInd).getSequenceStatus;
%                         disp('Sequence Status 2:');
%                         disp(XXXXX);
%                     end
%                     pause(1);
%                      uniqueStimInd=1%:length(uniqueStimulators)%use this code to interleave pulses between electrodes on a given CereStim (and to a lesser extent, between electrodes on different CereStims)
%                         stimulatorInd=find(stimulatorNums==uniqueStimulators(uniqueStimInd));            
%                         XXXXX = stimulator(stimulatorInd).getSequenceStatus;
%                         disp('Sequence Status:');
%                         disp(XXXXX);
...
%     SCNT(5) = { ['Electrode: ' num2str(electrode) ] };
    ...
    if visualTrial==0&&stimPattern<11        
        for uniqueStimInd=1:length(uniqueStimulators)
            stimulatorInd=find(stimulatorNums==uniqueStimulators(uniqueStimInd));
